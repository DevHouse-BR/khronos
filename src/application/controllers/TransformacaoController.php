			...
			try {
				...
				if ($maquina) {
					echo Zend_Json::encode(array('success' => true, 'data' => array(
						...
						'vl_credito_ant' => Khronos_Moeda::format($maquina->vl_credito),
		...
		if (DMG_Acl::canAccess(28)) {
			...
			try {
				$maquina = Doctrine_Query::create()
					...
					->fetchOne();
				if (!$maquina) {
					throw new Exception(DMG_Translate::_('transformacao.maquina.invalid'));
				}
				if ($maquina->ScmStatusMaquina->fl_permite_transformacao == 0) {
					throw new Exception(DMG_Translate::_('transformacao.status.nao-permite'));
				}
				...
				try {
					...
					throw new Exception(DMG_Translate::_('transformacao.data.erro'));
				}
				...
				for ($i = 1; $i <= 6; $i++) {
					...
					if (in_array($i, $cont)) {
						if (!$notEmpty->isValid($this->getRequest()->getParam($_nm))) {
							$error[$_nm] = DMG_Translate::_('parque.transformacao.form.contador.empty');
						}
					}
					if ($notEmpty->isValid($this->getRequest()->getParam($_nm))) {
						if (!$Int->isValid($this->getRequest()->getParam($_nm))) {
							$error[$_nm] = DMG_Translate::_('parque.transformacao.form.contador.string');
						}
						$trItem->$_nm = $this->getRequest()->getParam($_nm);
					}
					if ($trItem->$_nm < 0) {
						$error[$_nm] = DMG_Translate::_('parque.transformacao.form.contador.negativo');
					}
				}
				for ($i = 1; $i <= 6; $i++) {
					$_nm = 'nr_cont_' . $i . '_ant';
					...
					if ($notEmpty->isValid($this->getRequest()->getParam($_nm))) {
						if ($this->getRequest()->getParam($_nm) != $maquina->{'nr_cont_' . $i}) {
 							$error[$_nm] = DMG_Translate::_('parque.transformacao.form.contador.diferente');
						}
						if (!$Int->isValid($this->getRequest()->getParam($_nm))) {
 							$error[$_nm] = DMG_Translate::_('parque.transformacao.form.contador.string');
						}
